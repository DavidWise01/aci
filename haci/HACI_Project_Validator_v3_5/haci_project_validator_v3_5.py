...
v3.5 fixes:
1. Keeps missing parent scope gate from v3.4.
2. Purges dependency edges that touch blocked symbols.
3. Purges dependency edges to uncommitted target symbols.
4. Keeps edges from structural/source files to valid committed targets.
5. Cycle detection runs after edge purge.
"""
...
def is_symbol_blocked(sym: Optional[dict]) -> bool:
    if not sym:
        return False
    state = sym.get("commit_state", "")
    return isinstance(state, str) and state.startswith("blocked_")

def edge_endpoint_allowed(symbol: str, symbols: Dict[str, dict], is_source: bool) -> bool:
    sym = symbols.get(symbol)
    if sym is None:
        return False

    # Blocked symbols are invalid graph endpoints on either side.
    if is_symbol_blocked(sym):
        return False

    # Source files may be structural/uncommitted containers that ask/observe other objects.
    # This preserves the existing "real ask edge" behavior.
    if is_source:
        return True

    # Targets must be committed. A graph must not point into pending/uncommitted scope.
    return is_symbol_committed(sym)

def purge_blocked_symbol_edges(
    ...
    for edge in edge_records:
        src = edge.get("from")
        dst = edge.get("to")
        src_ok = edge_endpoint_allowed(src, symbols, is_source=True)
        dst_ok = edge_endpoint_allowed(dst, symbols, is_source=False)
        ...
        reasons = []
        if not src_ok:
            reasons.append(f"from:{src}")
        if not dst_ok:
            reasons.append(f"to:{dst}")
        warnings.append(
            f"{edge.get('file', src)}:L{edge.get('line', '?')}:BLOCKED_SYMBOL_EDGE_PURGED:"
            + ",".join(reasons)
        )
...
def validate_project(root: Path, strict: bool = False) -> ProjectResult:
    ...
    edge_records, purged_edge_warnings = purge_blocked_symbol_edges(edge_records, symbols)
    warnings.extend(purged_edge_warnings)
    edges = [(e["from"], e["to"]) for e in edge_records]
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v3.5")
