...
v2.2 fixes:
1. Enforces real dot-path resolution: dotted paths must exist as scoped files/objects.
2. Improves return pairing beyond object-only FIFO using payload-token scoring.
3. Strict mode fails unmatched returns.
4. Markdown headings and blank lines stop body attachment.
5. Object-required protocol lines remain required and are explicitly validated.
"""
...
def group_conversations(nodes: List[Node]) -> Tuple[List[Conversation], List[str], List[str]]:
    """
    v2.2 conversation engine:
    - Declarations complete by default.
    - Dual-edge lines complete by default and may still receive body lines.
    - Body attachment stops at headings, blank boundaries, code blocks, or next protocol line.
    - Return-only nodes are paired in a two-pass, order-independent pass.
    - Pairing uses object + token-overlap scoring; fallback only if exactly one ask and one return.
    """
    errors: List[str] = []
    warnings: List[str] = []
    ...
    for n in nodes:
        if n.owner in {"structure", "boundary", "code"}:
            # v2.2: structure/boundary/code stops body attachment.
            last_attachable_by_file[n.file] = None
            continue

        if is_protocol_node(n):
            # Object-required protocol lines.
            if not n.object:
    ...
    # Pair returns to unresolved conversations by object + payload score.
    ...
    for key, returns in returns_by_object.items():
        ...
        # Safe fallback: exactly one unresolved conv and one unmatched return for this object.
        ...
        if len(remaining_convs) == 1 and len(remaining_returns) == 1:
            conv = convs[remaining_convs[0]]
            ret = returns[remaining_returns[0]]
            conv.returns.append(node_to_dict(ret))
            conv.complete = True
            conv.unresolved = False
            conv.diagnostics.append("WEAK_RETURN_PAIR")
            warnings.append(f"{ret.file}:L{ret.line}:WEAK_RETURN_PAIR:{conv.id}")
            matched_convs.add(remaining_convs[0])
            matched_returns.add(remaining_returns[0])
        else:
            # Multiple zero-score possibilities remain ambiguous; do not pair.
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v2.2")
