#!/usr/bin/env python3
"""
HACI Validator v3.0 — Audit to Failure

No new features.
Goal: find the next meaningful semantic/protocol failure in v3.0.
"""
...
def check_prior_v29_chronology_fixed():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if r.ok or not has_error(r, "SAME_FILE_RETURN_BEFORE_ASK"):
            return save_failure(
                "prior v2.9 same-file chronology failure still present",
                root, r,
                "Same-file return before ask was not rejected.",
                "strict failure with SAME_FILE_RETURN_BEFORE_ASK"
            )
    return None

def check_cross_file_order_still_allowed():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        if not r.ok:
            return save_failure(
                "cross-file order-independent pairing regressed",
                root, r,
                "Cross-file return before ask was rejected, but cross-file merge pairing should remain order-independent.",
                "strict pass"
            )
    return None

def check_multiple_return_questions_same_ask():
    """
    A conversation can legitimately receive more than one returned question before a final answer.
    v3.0 likely matches only one suffix-? return-question per conversation because matched_convs prevents a second one.
    """
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
...
""")
        r = validate_project(root, strict=True)
        convs = ask_convs(r)
        alpha = convs[0] if convs else {}
        q_count = sum(1 for ret in alpha.get("returns", []) if ret.get("inbound") == "ask")
        if not r.ok or q_count != 2:
            return save_failure(
                "multiple return-questions for one ask rejected",
                root, r,
                "Only one suffix-? return-question can attach to a conversation; the second becomes unmatched even though it has the same object and positive payload relation.",
                "strict pass with both return-questions preserved, final answer completes"
            )
    return None

def check_multiple_return_questions_cross_file_same_ask():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "00_scope/m.haci", "! m memory >")
        ...
        if not r.ok or q_count != 2:
            return save_failure(
                "multiple cross-file return-questions for one ask rejected",
                root, r,
                "Multiple suffix-? return-questions from separate files could not attach to the same ask.",
                "strict pass with both return-questions preserved"
            )
    return None
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if edge_exists(r, "memory.m", "code.c"):
            return save_failure(
                ...
                root, r,
                "Pending declaration created a dependency edge again.",
                "no graph edge"
            )
    return None

def check_return_question_after_completion_still_fails():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "10_memory/m.haci", """? m alpha problem
m alpha answer >
...
""")
        ...
        if r.ok:
            return save_failure(
                ...
                root, r,
                "Return-question after completed answer passed strict mode.",
                "strict failure"
            )
    return None
...
def check_foreign_pending_declaration_does_not_mutate_authority():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "10_memory/m.haci", "! c pending ?")
        write(root, "20_code/c.haci", "! c code >")
        r = validate_project(root, strict=False)
        sym = r.symbols.get("code.c", {})
        if sym.get("meaning") != "code" or sym.get("authority") != "human":
            return save_failure(
                "foreign pending declaration mutated target authority",
                root, r,
                "Pending foreign declaration changed committed symbol authority/meaning.",
                "target symbol remains code/human from committed declaration"
            )
    return None

def deterministic_heavy(rounds=12000, seed=30030030):
    ...
    words = [
        ...
        "clarify", "problem", "answer", "scope", "detail"
    ]
    for i in range(rounds):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lines = []
            for _ in range(rng.randint(1, 38)):
...
def main():
    checks = [
        check_prior_v29_chronology_fixed,
        check_cross_file_order_still_allowed,
        check_multiple_return_questions_same_ask,
        check_multiple_return_questions_cross_file_same_ask,
        check_pending_decl_edge_still_fixed,
        check_return_question_after_completion_still_fails,
        check_duplicate_identical_asks_returns_still_fail,
        check_foreign_pending_declaration_does_not_mutate_authority,
    ...
    summary = {"failure": None, "message": "No failure found in current v3.0 audit-to-failure run."}
    Path("FAILURE_SUMMARY.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))
    return 0
