...
import tempfile
from haci_project_validator_v2_4 import validate_project, validate_merge
...
def test_return_question_does_not_complete_prior_ask():
    with tempfile.TemporaryDirectory() as tmp:
...
def test_declaration_inbound_question_is_open():
    with tempfile.TemporaryDirectory() as tmp:
...
def test_declaration_inbound_observe_still_passes():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory >")
        r = validate_project(Path(tmp), strict=True)
        assert r.ok, r.errors

# v2.3 preservation tests
...
if __name__ == "__main__":
    tests = [
        ...
        test_return_question_does_not_complete_prior_ask,
        test_return_observe_completes_prior_ask,
        test_declaration_inbound_question_is_open,
        test_declaration_inbound_observe_still_passes,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v2.4 tests passed")
