...
import tempfile
from haci_project_validator_v3_0 import validate_project, validate_merge
...
def test_cross_file_accepted_before_ask_allowed():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "00_scope/m.haci", "! m memory >")
        write(tmp, "10_returns/returns.haci", "m accepted !")
        write(tmp, "20_runtime/ask.haci", "? m approve deployment")
        r = validate_project(Path(tmp), strict=True)
        assert r.ok, r.errors
...
if __name__ == "__main__":
    tests = [
        ...
        test_cross_file_accepted_before_ask_allowed,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v3.0 tests passed")
