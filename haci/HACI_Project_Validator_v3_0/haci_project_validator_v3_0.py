...
v3.0 fixes:
1. Keeps pending-declaration edge gate from v2.9.
2. Adds same-file chronology gate.
3. Same-file returns may not pair backward to later asks.
4. Cross-file return/ask pairing remains order-independent.
5. Applies to >, !, and ? return-side nodes.
"""
    ...
    for key, returns in returns_by_object.items():
        ...
        if len(remaining_convs) == 1 and len(remaining_returns) == 1:
            ...
            if (
                ...
                and chronology_pair_allowed(conv, ret)
            ):
                conv.returns.append(node_to_dict(ret))
                conv.complete = True
                conv.unresolved = False
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v3.0")
