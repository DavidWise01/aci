...
import tempfile
from haci_project_validator_v2_6 import validate_project, validate_merge
...
# v2.6 accepted-return pairing tests
...
""")
        ...
        assert conv["unresolved"] is False
...
def test_multiple_remaining_returns_with_acceptance_still_fails():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m approve deployment
m accepted !
m unrelated >
""")
        r = validate_project(Path(tmp), strict=True)
        assert not r.ok
        assert has_error(r, "OPEN_CONVERSATION") or has_error(r, "RETURN_WITHOUT_OPEN_REQUEST")
...
if __name__ == "__main__":
    tests = [
        ...
        test_multiple_remaining_returns_with_acceptance_still_fails,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v2.6 tests passed")
