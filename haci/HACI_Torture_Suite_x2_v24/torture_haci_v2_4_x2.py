...
v2.4-specific:
- suffix > completes as returned answer/evidence
...
def b_declare_question_open(root):
    write(root, "10_memory/m.haci", "! m memory ?")

def c_declare_question_open(name, r):
    ok = not r.ok and has_error(r, "INBOUND_QUESTION_UNRESOLVED") and has_error(r, "OPEN_CONVERSATION")
    assert_true(name, ok, str(r.errors))
...
CURATED = [
    ...
    ("declare question open", b_declare_question_open, c_declare_question_open, True),
...
def build_random_project(root: Path, rng: random.Random, valid_bias=True):
    ...
    for _ in range(rng.randint(3, 10)):
        ...
        suffix = rng.choice([">", "?", "!", ""])
        if op == ">":
            line = f"{obj} {payload} {suffix}".rstrip()
        else:
            line = f"{op} {obj} {payload} {suffix}".rstrip()
        lines.append(line)
...
def main():
    ...
    try:
        ...
        report = {
            "suite": "HACI Torture Suite x2 for v2.4",
            "pass": PASS,
            "fail": FAIL,
