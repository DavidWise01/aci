...
v2.4 fixes:
1. Keeps no-guess pairing from v2.3.
2. Enforces inbound suffix semantics.
3. Suffix > completes as told/evidence/result.
4. Suffix ! completes as accepted/committed.
5. Suffix ? stays unresolved/open and fails strict mode.
"""
    ...
    for n in nodes:
        ...
        if is_protocol_node(n):
            ...
            if n.outbound == "declare":
                unresolved = inbound_is_question(n.inbound)
                conv = make_conversation(n, "declaration", complete=not unresolved, unresolved=unresolved)
                if unresolved:
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v2.4")
