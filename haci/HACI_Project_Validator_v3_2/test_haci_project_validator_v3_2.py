...
import tempfile
from haci_project_validator_v3_2 import validate_project, validate_merge
...
# v3.2 multi evidence/result return tests
...
def test_multiple_evidence_returns_distribute_to_unique_asks():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m alpha red problem
? m beta blue problem
...
""")
        ...
        assert count_returns(alpha, "observe") == 1, alpha
        assert count_returns(beta, "observe") == 1, beta
...
def test_zero_score_evidence_return_still_fails_closed():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m alpha problem
m gamma evidence >
""")
...
def test_multiple_evidence_then_return_question_after_completion_fails():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m alpha problem
m alpha evidence one >
m alpha evidence two >
...
def test_multiple_evidence_plus_accepted_extra_bang_fails_closed():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m alpha problem
m alpha evidence one >
m alpha evidence two >
...
# v3.1 multi return-question preservation
...
def test_multiple_return_questions_then_accept_pass():
    with tempfile.TemporaryDirectory() as tmp:
...
""")
        ...
        assert count_returns(alpha, "ask") == 1, alpha
        assert count_returns(alpha, "observe") == 1, alpha
        assert count_returns(beta, "observe") == 1, beta
...
if __name__ == "__main__":
    tests = [
        ...
        test_multiple_evidence_returns_distribute_to_unique_asks,
        test_ambiguous_evidence_return_still_fails_closed,
        test_zero_score_evidence_return_still_fails_closed,
        test_multiple_evidence_then_return_question_after_completion_fails,
        test_multiple_evidence_plus_accepted_extra_bang_fails_closed,
        test_multiple_return_questions_same_ask_pass,
        test_multiple_return_questions_then_accept_pass,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v3.2 tests passed")
