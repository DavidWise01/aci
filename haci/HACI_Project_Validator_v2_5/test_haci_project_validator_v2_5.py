...
import tempfile
from haci_project_validator_v2_5 import validate_project, validate_merge
...
def decl_convs(r):
    return [c for c in r.conversations if c["kind"] == "declaration"]
...
def test_committed_bang_declaration_sets_symbol():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory !")
        r = validate_project(Path(tmp), strict=True)
        assert r.ok, r.errors
...
def test_pending_declaration_does_not_poison_later_valid_declaration_same_meaning():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """! m memory ?
! m memory >
""")
        ...
        assert r.symbols["memory.m"].get("meaning") == "memory"
...
def test_return_question_then_answer_completes():
    with tempfile.TemporaryDirectory() as tmp:
...
""")
        ...
        assert any(ret["inbound"] == "observe" for ret in conv["returns"])
...
def test_single_no_overlap_does_not_weak_pair():
    with tempfile.TemporaryDirectory() as tmp:
...
if __name__ == "__main__":
    tests = [
        ...
        test_committed_bang_declaration_sets_symbol,
        test_pending_declaration_does_not_poison_later_valid_declaration_same_meaning,
        ...
        test_return_question_then_answer_completes,
        ...
        test_single_no_overlap_does_not_weak_pair,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v2.5 tests passed")
