...
v3.2 fixes:
1. Keeps multi return-question attachment from v3.1.
2. Allows multiple suffix > evidence/result returns to attach to one conversation.
3. Each > return still needs a positive unique conversation match.
4. Suffix > returns complete the conversation; multiple evidence lines are preserved.
5. Suffix ! accepted-return fallback remains conservative.
"""
...
def group_conversations(nodes: List[Node]) -> Tuple[List[Conversation], List[str], List[str]]:
    ...
    for key, returns in returns_by_object.items():
        ...
        # v3.2: suffix > evidence/result returns attach independently.
        # Multiple > returns may attach to the same conversation, but each return must
        # have exactly one positive best conversation match.
        for ri, ret in enumerate(returns):
            if ret.inbound != "observe":
                continue
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v3.2")
