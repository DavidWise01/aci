...
v2.3 fixes:
1. Keeps real dot-path resolution from v2.2.
2. Removes optimistic return guessing.
3. Pairs returns only when the match is mutual, unique, positive best.
4. Equal-score ambiguity stays unresolved and fails strict mode.
5. Strict mode still fails unmatched returns.
"""
...
def group_conversations(nodes: List[Node]) -> Tuple[List[Conversation], List[str], List[str]]:
    ...
    for key, returns in returns_by_object.items():
        ...
        # v2.3 no-guess pairing:
        # Pair only when a return is the mutual, unique, positive best match.
        # Equal-score ambiguity is not resolved by file order, line order, or FIFO.
        changed = True
        while changed:
            changed = False
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v2.3")
