...
import tempfile
from haci_project_validator_v2_7 import validate_project, validate_merge
...
# v2.7 pending-parent scope gate tests
...
def test_pending_parent_blocks_child_commit_even_loose():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        parent = r.symbols["memory.m"]
        child = r.symbols["memory.m.parser.last"]
        assert "meaning" not in parent, parent
...
def test_pending_parent_blocks_child_commit_strict():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory ?")
        write(tmp, "10_memory/m/parser/last.haci", "! last prior >")
...
def test_uncommitted_parent_without_declaration_blocks_child():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "plain body only")
        write(tmp, "10_memory/m/parser/last.haci", "! last prior >")
        r = validate_project(Path(tmp), strict=False)
        assert not r.ok
        assert has_error(r, "PENDING_PARENT_SCOPE"), r.errors
        assert r.symbols["memory.m.parser.last"].get("commit_state") == "blocked_pending_parent"
...
def test_pending_sibling_does_not_block_child_when_parent_committed():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory >\n! m maybe ?")
        write(tmp, "10_memory/m/parser/last.haci", "! last prior >")
        r = validate_project(Path(tmp), strict=True)
        # strict fails due pending second declaration, but not due parent-scope block;
        # parent remains committed because first declaration committed.
        assert not r.ok
        assert not has_error(r, "PENDING_PARENT_SCOPE"), r.errors
        assert r.symbols["memory.m"].get("commit_state") == "committed"
        assert r.symbols["memory.m.parser.last"].get("commit_state") == "committed"
...
def test_return_question_then_accepted_commit_closes():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """? m approve deployment
m clarify ?
m accepted !
""")
        ...
        assert r.ok, r.errors
        conv = ask_convs(r)[0]
        assert any(ret["inbound"] == "ask" for ret in conv["returns"])
        assert any(ret["inbound"] == "declare" for ret in conv["returns"])
        assert has_diag(conv, "ACCEPTED_RETURN_PAIR"), conv
...
def test_no_suffix_declaration_pending_not_committed():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory")
        r = validate_project(Path(tmp), strict=True)
        assert not r.ok
        assert has_error(r, "DECLARATION_PENDING_NOT_COMMITTED"), r.errors
        sym = r.symbols["memory.m"]
        assert "meaning" not in sym
        assert "authority" not in sym
...
def test_pending_declaration_does_not_conflict_later_different_valid_declaration():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", """! m memory ?
! m model >
""")
        r = validate_project(Path(tmp), strict=True)
        assert not r.ok
        assert has_error(r, "DECLARATION_PENDING_NOT_COMMITTED")
        assert not has_error(r, "DUPLICATE_OR_CONFLICTING_SYMBOL"), r.errors
        assert not has_error(r, "AUTHORITY_MUTATION"), r.errors
        assert r.symbols["memory.m"].get("meaning") == "model"
...
def test_single_no_overlap_observe_does_not_weak_pair():
    with tempfile.TemporaryDirectory() as tmp:
...
if __name__ == "__main__":
    tests = [
        ...
        test_pending_parent_blocks_child_commit_strict,
        test_uncommitted_parent_without_declaration_blocks_child,
        test_committed_parent_allows_child_commit,
        test_pending_sibling_does_not_block_child_when_parent_committed,
        ...
        test_return_question_then_accepted_commit_closes,
        test_unresolved_question_declaration_does_not_commit_symbol_authority,
        test_no_suffix_declaration_pending_not_committed,
        test_committed_observe_declaration_sets_symbol,
        test_pending_declaration_does_not_conflict_later_different_valid_declaration,
        ...
        test_single_no_overlap_observe_does_not_weak_pair,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v2.7 tests passed")
