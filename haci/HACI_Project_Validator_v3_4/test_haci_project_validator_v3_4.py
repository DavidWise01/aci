...
import tempfile
from haci_project_validator_v3_4 import validate_project, validate_merge
...
def test_committed_parent_chain_allows_child_commit():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert r.symbols["memory.m.parser.last"]["meaning"] == "prior"
...
def test_pending_parent_still_blocks_child_commit():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory ?")
        write(tmp, "10_memory/m/parser.haci", "! parser parser >")
...
def test_pending_intermediate_parent_blocks_deep_child_commit():
    with tempfile.TemporaryDirectory() as tmp:
        write(tmp, "10_memory/m.haci", "! m memory >")
        write(tmp, "10_memory/m/parser.haci", "! parser parser ?")
        write(tmp, "10_memory/m/parser/last.haci", "! last prior >")
...
# v3.3 multi commit preservation
...
if __name__ == "__main__":
    tests = [
        ...
        test_pending_parent_still_blocks_child_commit,
        test_pending_intermediate_parent_blocks_deep_child_commit,
    ...
    for t in tests:
        t()
    print("PASS: HACI Project Validator v3.4 tests passed")
