#!/usr/bin/env python3
"""
HACI Validator v3.1 — Torture to Failure

No new features.
Goal: find the next meaningful semantic/protocol failure in v3.1.
"""
...
def check_prior_v30_multi_question_fixed():
...
""")
        ...
        convs = ask_convs(r)
        if not r.ok or not convs or count_returns(convs[0], "ask") != 2:
            return save_failure(
                "prior v3.0 multi-return-question failure still present",
                root, r,
                "Multiple suffix ? return-questions did not attach to one open ask.",
                "strict pass with two return-questions and final answer"
            )
    return None

def check_multiple_evidence_returns_same_ask():
    """
    One ask can reasonably receive multiple returned statements/evidence lines before the conversation closes.
    v3.1 uses one-completion-per-conversation matching and treats multiple > returns as ambiguous/unmatched.
    """
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "10_memory/m.haci", """? m alpha problem
...
""")
        ...
        obs = count_returns(conv, "observe")
        if not r.ok or obs != 2:
            return save_failure(
                "multiple evidence returns for one ask rejected",
                root, r,
                "Only one suffix > completion can attach to a conversation; multiple evidence/result returns become ambiguous or unmatched even when all belong to the same ask.",
                "strict pass with both suffix > returns preserved on the ask"
            )
    return None

def check_multiple_cross_file_evidence_returns_same_ask():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "00_scope/m.haci", "! m memory >")
        write(root, "10_runtime/ask.haci", "? m alpha problem")
        write(root, "20_returns/e1.haci", "m alpha evidence one >")
        write(root, "21_returns/e2.haci", "m alpha evidence two >")
        r = validate_project(root, strict=True)
        convs = ask_convs(r)
        conv = convs[0] if convs else {}
        if not r.ok or count_returns(conv, "observe") != 2:
            return save_failure(
                "multiple cross-file evidence returns for one ask rejected",
                root, r,
                "Multiple suffix > returns from separate files could not attach to one open ask.",
                "strict pass with both evidence returns preserved"
            )
    return None

def check_multiple_evidence_distribute_to_unique_asks():
    """
    Positive control: if returns are for separate asks, unique matching should still work.
    """
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        write(root, "10_memory/m.haci", """? m alpha red problem
? m beta blue problem
m alpha red evidence >
m beta blue evidence >
""")
        r = validate_project(root, strict=True)
        if not r.ok:
            return save_failure(
                "unique evidence returns stopped distributing to separate asks",
                root, r,
                "Separate suffix > returns no longer matched separate asks.",
                "strict pass"
            )
    return None
...
def deterministic_heavy(rounds=14000, seed=31031031):
    ...
    words = [
        ...
        "clarify", "problem", "answer", "scope", "detail", "evidence"
    ]
...
def main():
    checks = [
        check_prior_v30_multi_question_fixed,
        check_multiple_evidence_returns_same_ask,
        check_multiple_cross_file_evidence_returns_same_ask,
        check_multiple_evidence_distribute_to_unique_asks,
        ...
        lambda: deterministic_heavy(14000),
    ]
    ...
    summary = {"failure": None, "message": "No failure found in current v3.1 torture-to-failure run."}
    Path("FAILURE_SUMMARY.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))
    return 0
