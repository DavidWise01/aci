...
v2.8 fixes:
1. Keeps pending-parent scope gate from v2.7.
2. Adds payload-specific return-question pairing.
3. Suffix ? return-questions use mutual unique positive-best pairing.
4. Return-questions attach without completing the conversation.
5. Ambiguous or unmatched return-questions still fail closed.
"""
...
def group_conversations(nodes: List[Node]) -> Tuple[List[Conversation], List[str], List[str]]:
    ...
    # v2.8: Attach suffix-? return-question nodes by mutual unique positive-best match.
    ...
    for key, questions in return_questions_by_object.items():
        ...
        while changed:
            ...
            remaining_questions = [i for i in range(len(questions)) if i not in matched_questions]
            if not remaining_convs or not remaining_questions:
                break

            scores: Dict[Tuple[int, int], int] = {}
            for ci in remaining_convs:
                for qi in remaining_questions:
                    scores[(ci, qi)] = pairing_score(convs[ci], questions[qi])

            for ci in list(remaining_convs):
                conv_scores = [(scores[(ci, qi)], qi) for qi in remaining_questions]
                best_score = max(score for score, _ in conv_scores) if conv_scores else 0
                if best_score <= 0:
                    continue

                best_questions = [qi for score, qi in conv_scores if score == best_score]
                if len(best_questions) != 1:
                    convs[ci].diagnostics.append("AMBIGUOUS_RETURN_QUESTION_PAIR")
                    continue

                qi = best_questions[0]
                q_scores = [(scores[(cj, qi)], cj) for cj in remaining_convs]
                q_best_score = max(score for score, _ in q_scores) if q_scores else 0
                q_best_convs = [cj for score, cj in q_scores if score == q_best_score]

                if q_best_score == best_score and len(q_best_convs) == 1 and q_best_convs[0] == ci:
                    conv = convs[ci]
                    retq = questions[qi]
    ...
    for key, returns in returns_by_object.items():
        ...
        # v2.6 accepted-return fallback:
        # Suffix ! means accepted / acknowledged / committed back.
        # It may close without token overlap only when there is exactly one open same-object ask
        # and exactly one remaining same-object return, and that return is !.
        # This avoids guessing between duplicate/multiple asks or multiple returns.
        if len(remaining_convs) == 1 and len(remaining_returns) == 1:
            ci = remaining_convs[0]
            ri = remaining_returns[0]
            conv = convs[ci]
            ret = returns[ri]
            if (
                conv.kind == "conversation"
                and conv.start.get("outbound") == "ask"
                and ret.inbound == "declare"
            ):
                conv.returns.append(node_to_dict(ret))
                conv.complete = True
                conv.unresolved = False
...
def apply_pending_parent_scope_gate(symbols: Dict[str, dict]) -> List[str]:
    """
    Structural gate:
    If an existing parent symbol is uncommitted/pending, a child symbol may not remain committed beneath it.
    ...
    for symbol in sorted(list(symbols.keys())):
        ...
        if not is_symbol_committed(sym):
            continue
        parent = nearest_existing_parent_symbol(symbol, symbols)
        if not parent:
            continue
        parent_sym = symbols[parent]
        if is_symbol_committed(parent_sym):
            continue
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="HACI Project Validator v2.8")
